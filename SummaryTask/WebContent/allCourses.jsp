					...
					<table class="table table-borderless">
						<tr>
							<td scope="col"><label for="exampleInputEmail1"><fmt:message key="courses.label.startDate" />
								...
								id="exampleInputEmail1" value=${course.getStartDate() } readonly>
							</td>
							<td scope="col"><label for="exampleInputEmail1"><fmt:message key="courses.label.endDate" />
							</label> <input type="date" class="form-control"
								id="exampleInputEmail1" value=${course.getEndDate() } readonly></td>
						</tr>
						<tr>
					...
					<c:if test="${not empty user}">
						...
						<c:if test="${userRole.name == 'student'}">
							<form action="controller" method="post">
								...
								<c:if test="${course.getStartDate() le course.getCurrentDate()}">
								...
								<c:if test="${course.getStartDate() gt course.getCurrentDate()}">
						...
						<c:if test="${userRole.name == 'admin'}">
							<div class="m-1">
								<form action="controller" method="post">
									<a
										href="controller?command=editCourse&courseId=${course.getId()}"
										class="btn btn-primary m-1"><fmt:message key="courses.button.edit" /></a> <a
										href="controller?command=deleteCourse&courseId=${course.getId()}"
